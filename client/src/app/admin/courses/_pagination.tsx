...
export default function PaginationBar({
  page, totalPages, total, limit, onPrev, onNext, onJump,
}: Props) {
  const start = total ? (page - 1) * limit + 1 : 0;
  const end = Math.min(total || page * limit, page * limit);
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between p-3">
      <div className="text-xs text-white/70">
        Menampilkan <span className="font-medium text-white/90">{start}</span>
        ...
        {pages.map((p) => (
          <button
            ...
            className={[
              ...
              p === page ? "border-white/20 bg-white/15 text-white"
